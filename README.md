...
| Strona | Zawartość |
|---|---|
| **Stan konta** *(od 0.21.0, zastąpiła dawny Pulpit)* | Odpowiedź na „jaki jest mój stan": sticky pasek z wartością portfela i saldem u brokera; karta portfela **(od 0.8.0 jako kafelki: hero „Wartość całkowita” na górze [PLN duże, EUR jako druga linia] nad trzema kubełkami — „Wolne” (można sprzedać), „Z ograniczeniem” (widoczny tylko gdy dotyczy), „Zablokowane” (nienabyte dopasowania ESPP/transze LTI, z najbliższą datą dostępności) — plus pasek wyniku (koszt bazowy/P&L/całkowity zwrot/dywidendy netto) i ostrzeżenie o zaległych transzach)**, donut trzech kubełków portfela *(od 0.12.0)*; karta „Gotówka i podatek” (skrót salda brokera/wpływów ze sprzedaży/podatku do zapłaty/dywidend, linki do `/gotowka` i `/pit38`); karta „Najbliższe zdarzenia” *(od 0.21.0 — vesting, dywidenda, koniec restrykcji ESPP, termin PIT-38 na jednej osi czasu, posortowane po dacie)*; karta „Ile wypłacę?” (skrót do symulacji sprzedaży na **Plan**); sekcja „Dziś warto wiedzieć” *(od 0.12.0, deterministyczna — bez AI)* — zmiana kursu dziś, najbliższy vesting, sygnał podatkowy; pole szybkiego pytania do **Asystenta** *(od 0.13.0)* |
| **Rynek** *(od 0.21.0, dawny Pulpit — kurs/wykres/AI bez zmian treści)* | Sticky pasek *(od 0.12.0)* z ceną widoczny cały czas przy przewijaniu; kurs (EUR, **od 0.5.0** z linią `≈ X zł` po kursie bieżącym), zmiana dzienna, sesja, trend, RSI, wykres cenowy z konfigurowalnym zakresem (1D/1W/1M/3M/6M/1R/3L/5L/MAX, wybór zapamiętany), sentyment i briefing AI, rekomendacja AI, prognozy 1w/1m/12m, ostatnie alerty, przycisk „Przeanalizuj teraz” |
...
| **Gotówka** *(od 0.20.0)* | Księga gotówki — model odczytu nad `sales`/`dividends`/PIT-38, zero nowej matematyki podatkowej. Karta „Saldo u brokera” (wpis ręczny — wyciąg Computershare nie zawiera pola salda gotówkowego, sprawdzone empirycznie; historia odczytów z wiekiem, ostrzeżenie po 30 dniach bez aktualizacji); „Wpływy ze sprzedaży” (rok + narastająco, liczone z realnego `revenue_pln`/kursu NBP zapisanego przy sprzedaży, nie z ceny nominalnej — nie gubi override „Sale Proceeds”); „Podatek PIT-38” (należny vs faktycznie zapłacone wpłaty, saldo, termin 30.04); „Dywidendy — reinwestowane, bezgotówkowe” (wszystkie dywidendy w produkcji są DRIP — przychód podatkowy owszem, ale zerowy wpływ gotówki, pokazane osobno i jawnie oznaczone) |
...
| **Kopia zapasowa** *(od 0.8.1)* | Eksport pełnego zrzutu bazy (ZIP: `nokia.db` + manifest + CSV lotów/sprzedaży/grantów/transz/dywidend), przywracanie z podglądem różnicy per tabela przed zapisem (nigdy nie nadpisuje w ciemno), informacja o ostatniej kopii nocnej i liczbie nierozstrzygniętych konfliktów importu, karta **„Spójność danych”** *(od 0.18.0 — 9 niezmienników nad `lots`/`sales`/`dividends`/`vests`/`import_conflicts`/stratami z lat ubiegłych, sprawdzane też co noc o 6:35 z alertem przy pierwszym wystąpieniu)* |
